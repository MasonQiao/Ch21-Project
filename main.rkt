...
(define (dh1 m) (place-image (circle 50 "solid" "red") 900 450 (crop (- (player_object-xpos m) 900) (- (player_object-ypos m) 450) 1800 900 spikes)))
...
(define (death? m)
  (cond [(or (equal? (get-pixel-color (floor (+ 50 (player_object-xpos m))) (floor (+ 50 (player_object-ypos m))) spikes) (make-color 255 136 136 255))
             ...
             (equal? (get-pixel-color (floor (+ 30 (player_object-xpos m))) (floor (+ -30 (player_object-ypos m))) spikes) (make-color 255 136 136 255))
             (equal? (get-pixel-color (floor (+ -30 (player_object-xpos m))) (floor (+ -30 (player_object-ypos m))) spikes) (make-color 255 136 136 255))
             (equal? (get-pixel-color (floor (+ -30 (player_object-xpos m))) (floor (+ -30 (player_object-ypos m))) spikes) (make-color 255 136 136 255))) false]
        [else true]))
